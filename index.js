	...
	await Promise.all(
		installedApps.map(async (installedApp) => {
			const modulePath = path.resolve(
				`./installs/${installedApp.name}/app.js`
			);
...
async function installApp(uri) {
	try {
		...
		const existingApp = apps.installed_apps.find(
			(a) => a.name === manifest.name
		);
		...
		const depPermissions = !hasDependencies
			...
			: manifest.depends_on.map((dep) => {
					const referencedDep = apps.installed_apps.find(
						(a) => a.manifest_uri === dep.manifest_uri
					);
		...
		if (!!existingApp) {
			apps.installed_apps = apps.installed_apps.filter(
				(a) => a.name !== manifest.name
			);
		}
